...
def dispatch_command(request: dict, repository: str, branch: str) -> list[str]:
    ...
    return ["gh", "workflow", "run", commit_build.WORKFLOW, "--repo", repository, "--ref", branch,
...
def configured_publisher(repository: str) -> ChannelPublisher:
    from scripts.releases.channel_qualification import configured_verifier

    store = R2ChannelStore(*r2.credentials())
    base = r2.public_base_url()
    # Resolve evidence policy lazily: listing and preview allocation do not need
    # qualification API calls, and CLI parsing must not import this dependency.
    def verify_build(request: dict, manifest: dict) -> bool:
        return configured_verifier(store, repository, base).verify_build(request, manifest)

    def verify_retirement(source: dict, target: dict, qualification: dict) -> bool:
        return configured_verifier(store, repository, base).verify_retirement(source, target, qualification)

    return ChannelPublisher(store, repository, base, authorize=maintainer_authorizer(repository),
                            verify_build=verify_build, verify_retirement=verify_retirement)
    ...
    try:
        ...
        operations = {
            ...
            "retire_channel": lambda: publisher.retire(args.retire_channel, args.to, args.minimum_version,
                                                        args.compatibility_key, args.compatibility_sha256,
...
def add_arguments(parser) -> None:
    ...
    parser.add_argument("--minimum-version", metavar="X.Y.Z", help="Qualified destination version floor")
    parser.add_argument("--compatibility-key", help="Immutable retirement qualification object key")
    parser.add_argument("--compatibility-sha256", type=require_sha256, help="Exact retirement qualification digest")
...
def validate_arguments(parser, args) -> bool:
    ...
    retirement = (args.to, args.minimum_version, args.compatibility_key, args.compatibility_sha256)
    ...
    if args.retire_channel and not all(retirement):
        parser.error("--retire-channel requires --to, --minimum-version, --compatibility-key and --compatibility-sha256")
