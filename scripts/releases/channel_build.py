...
from hermes_cli.release_channels import (
    ChannelError, validate_name, validate_repository,
)
...
def dispatch_command(name: str, commit: str, repository: str, branch: str,
                     bundle_env: dict | None = None) -> list[str]:
    validate_name(name)
    ...
    command = ["gh", "workflow", "run", commit_build.WORKFLOW, "--repo", repository, "--ref", branch,
               "-f", "channel=" + name, "-f", "build_commit=" + commit,
...
def prepare_build(*, name: str, revision: str, remote: str, repo: Path, repository: str,
                  default_branch: str, dispatch, bundle_env: dict | None = None,
...
def validate_arguments(parser, args) -> bool:
    selected = [key for key in ("channel", "channels", "retire_channel", "bootstrap_channels") if getattr(args, key)]
