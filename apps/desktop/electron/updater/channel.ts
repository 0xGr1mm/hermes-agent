...
function assertVersionFloor(version: string, floor: string): void {
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    if (current[index] < minimum[index]) { throw new Error('Destination is below retirement minimum version') }
  }
}
...
export class ChannelResolver {
  ...
  async read(key: string, digest?: string): Promise<string> {
    ...
    if (!response.ok || response.url !== url) { throw new Error(`Channel read unavailable: HTTP ${response.status}`) }
    const maximum = 4 * 1024 * 1024
    const reader = response.body?.getReader()
    if (!reader) { throw new Error('Channel metadata has no body') }
    ...
    try {
      while (true) {
        ...
        if (size > maximum) { throw new Error('Channel metadata exceeds size limit') }
        chunks.push(value)
      }
    } finally {
      await reader.cancel()
    ...
    if (digest && createHash('sha256').update(body).digest('hex') !== digest) { throw new Error('Channel metadata SHA256 mismatch') }
  ...
  private assertRecord(record: ActiveChannel | RetiredChannel, name: string): void {
    if (record.name !== name) { throw new Error('Channel name does not match object key') }
    if (record.repository.toLowerCase() !== this.deps.build.repository.toLowerCase()) { throw new Error('Channel repository authority mismatch') }
  }

  private async target(channel: ActiveChannel): Promise<ChannelTarget> {
    if (!channel.head) { throw new Error('No destination build published') }
    const manifest = decodeChannelManifest(await this.read(channel.head.manifestKey, channel.head.sha256))
    const request = manifest.request
    if (request.buildId !== channel.head.buildId || request.sequence !== channel.head.sequence || request.channel !== channel.name ||
        request.repository.toLowerCase() !== channel.repository.toLowerCase() || request.publicBase !== this.base ||
        !sameChannelIdentity(request.identity, channel.identity)) { throw new Error('Channel manifest request binding mismatch') }
    if (channel.policy === 'preview' && (request.version !== `0.0.${request.sequence}` ||
    ...
    const entry = manifest.packages.find((item: ChannelPackage): boolean => item.platform === this.deps.platform && item.arch === this.deps.arch)
    if (!entry) { throw new Error('Channel has no package for this platform and architecture') }
    const signer = entry.platform === 'darwin' ? entry.teamId : entry.publisher
    if (!this.deps.signer || signer !== this.deps.signer) { throw new Error('Channel package signing identity mismatch') }
    return {
      channel, manifest, package: entry, manifestSha256: channel.head.sha256,
      artifactUrl: `${this.base}/${entry.artifact.key}`, feedUrl: `${this.base}/${entry.feed.key}`
    }
  }
  ...
  private assertPackages(channel: ActiveChannel, manifest: ChannelManifest): void {
    ...
    if (channel.policy !== 'preview') {
      if (!request.releaseTag || request.version !== request.releaseTag.slice(1)) { throw new Error('Protected release version mismatch') }
    ...
    for (const entry of manifest.packages) {
      ...
      if (entry.identity !== expectedIdentity || entry.version !== expectedVersion || entry.feed.channel !== 'stable' ||
      ...
      if (entry.platform === 'win32' && !entry.feed.key.endsWith('.appinstaller')) { throw new Error('Invalid native Windows feed descriptor') }
    }
  }
}
