...
export interface ChannelHead { buildId: string; sequence: number; manifestKey: string; sha256: string }
...
function parseChannelJson(body: string): unknown {
  ...
  for (const match of body.matchAll(tokens)) {
    const token = match[0]
    if (token === '{') { objects.push(new Set<string>()); continue }
    if (token === '}') { objects.pop(); continue }
    if (!token.endsWith(':')) { continue }
    const key: string = JSON.parse(token.slice(0, -1))
    const keys = objects[objects.length - 1]
    if (keys.has(key)) { throw new Error('Duplicate channel JSON member') }
    keys.add(key)
  }
  return parsed
}
...
class Fields {
  ...
  constructor(input: unknown) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) { throw new Error('Expected channel object') }
    this.input = input
  }
  get(key: string): unknown { return Reflect.get(this.input, key) }
  keys(): string[] { return Object.keys(this.input) }
  text(key: string, pattern: RegExp = /^[\s\S]{1,2048}$/): string {
    const value: unknown = this.get(key)
    if (typeof value !== 'string' || [...value].some((character: string): boolean => character.charCodeAt(0) < 32) || !pattern.test(value)) { throw new Error(`Invalid channel ${key}`) }
    return value
  }
  ...
  object(key: string): Fields { return new Fields(this.get(key)) }
  optional(key: string, pattern?: RegExp): string | undefined {
    return this.get(key) === undefined ? undefined : this.text(key, pattern)
  }
  schema(): void { if (this.get('schema') !== 1) { throw new Error('Unsupported channel schema') } }
}
...
export function channelKey(value: string): string {
  if (!/^releases\/[A-Za-z0-9_./+-]+$/.test(value) ||
      value.split('/').some((part: string): boolean => !part || part === '.' || part === '..' || part.endsWith('.') || /^(con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part))) {
...
export function buildPrefix(buildId: string): string {
  if (!BUILD_ID.test(buildId)) { throw new Error('Invalid channel buildId') }
...
export function sameChannelIdentity(left: ChannelIdentity, right: ChannelIdentity): boolean {
  return left.token === right.token && left.displayName === right.displayName && left.appId === right.appId &&
    left.appNamePascal === right.appNamePascal && left.artifactNamePascal === right.artifactNamePascal &&
    left.cliName === right.cliName && left.windowsExecutableName === right.windowsExecutableName &&
...
function head(value: unknown): ChannelHead | null {
  ...
  if (manifestKey !== `${buildPrefix(buildId)}build.json`) { throw new Error('Invalid channel manifestKey') }
  return { buildId, manifestKey, sequence: fields.integer('sequence', 0xffffffff), sha256: fields.text('sha256', SHA256) }
}
...
export function decodeChannelRecord(body: string): ChannelRecord {
  ...
  const common: ChannelRecordBase = {
    schema: 1, name: validateChannelName(fields.text('name')),
    repository: fields.text('repository', /^[A-Za-z0-9][A-Za-z0-9_-]*\/[A-Za-z0-9][A-Za-z0-9_.-]*$/),
    policy, identity: identity(fields.object('identity')), revision: fields.integer('revision'),
    nextSequence: fields.integer('nextSequence', 0x100000000), head: head(fields.get('head'))
  }
  if (common.head && common.head.sequence >= common.nextSequence) { throw new Error('Channel head exceeds allocation') }
  const state = fields.text('state')
  if (state === 'active') { return { ...common, state } }
...
function request(fields: Fields): ChannelRequest {
  ...
  for (const key of environment.keys()) {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) { throw new Error('Invalid bundle environment name') }
    const value: unknown = environment.get(key)
    if (value !== null && (typeof value !== 'string' || value.includes('\0'))) { throw new Error('Invalid bundle environment value') }
  ...
  return {
    ...
    sequence: fields.integer('sequence', 0xffffffff), repository: fields.text('repository', /^[A-Za-z0-9][A-Za-z0-9_-]*\/[A-Za-z0-9][A-Za-z0-9_.-]*$/),
...
function packageEntry(value: unknown): ChannelPackage {
  ...
  if ((platform !== 'darwin' && platform !== 'win32') || (arch !== 'x64' && arch !== 'arm64') || fields.get('variant') !== 'bundled') {
...
export function decodeChannelManifest(body: string): ChannelManifest {
  ...
  if (!Array.isArray(entries) || !entries.length || entries.length > 4) { throw new Error('Invalid channel packages') }
