...
Each member's turn carries every room message since its own last turn — up to about 200 messages or 32 KB of transcript, whichever fills first — with a single oversized paste cut to a marked excerpt rather than crowding out the messages around it; when the window still overflows, the prompt names exactly how many earlier messages were left out.
