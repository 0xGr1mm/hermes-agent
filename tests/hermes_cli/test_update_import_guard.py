...
def test_import_guard_rejects_malformed_health_payload(monkeypatch, probe_root):
    class Result:
        returncode = 0
        stdout = ""
...
def test_import_guard_reports_probe_timeout(monkeypatch, probe_root):
    import subprocess
...
def test_untracked_enumeration_failure_is_visible(monkeypatch, probe_root, capsys):
    class Result:
        returncode = 1
        stdout = ""
    ...
    assert update_cmd._git_untracked_paths(["git"], probe_root) is None
